...
export type SbbDateFilterFn = (date: Date) => boolean;

/**
 * A {@link MetadataKey} representing a date filter for a date input.
 */
export const SBB_DATE_FILTER = createMetadataKey<SbbDateFilterFn | undefined>();

/**
 * An error used to indicate that a date does not pass the date filter validator.
 */
export class SbbDateFilterValidationError extends BaseNgValidationError {
  override readonly kind = 'sbbDateFilter';

  constructor(
    readonly dateFilter: SbbDateFilterFn,
 ...
 *   be a function that returns the date filter function.
 * @param config Optional, allows providing any of the following options:
 *  - `error`: Custom validation error(s) to be used instead of the default `SbbDateFilterValidationError`
...
export function dateFilter<TValue extends Date | null, TPathKind extends PathKind = PathKind.Root>(
  ...
  dateFilterValue: LogicFn<TValue, SbbDateFilterFn | undefined, TPathKind>,
  ...
  metadata(path, SBB_DATE_FILTER, ({ state }) => state.metadata(FILTER_MEMO)!());
  ...
  validate(path, (ctx) => {
    ...
    if (!dateFilter(value)) {
      if (config?.error) {
        ...
        return new SbbDateFilterValidationError(dateFilter, { message });
      }
    }
    return undefined;
  });
