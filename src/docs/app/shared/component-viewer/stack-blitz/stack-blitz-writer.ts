...
export class StackBlitzWriter {
   ...
   */
  async #buildInMemoryFileDictionary(data: ExampleData): Promise<FileDictionary> {
   ...
   */
  #replaceExamplePlaceholders(data: ExampleData, fileName: string, fileContent: string): string {
    ...
    if (fileName === 'src/index.html' || fileName === 'package.json') {
      fileContent = fileContent.replace(/\{\{\s?angular_version\s?}}/g, this.#version);
      fileContent = fileContent.replace(/\{\{\s?components_version\s?}}/g, this.#lyneVersion);
      fileContent = fileContent.replace(/\{\{\s?token_version\s?}}/g, this.#tokenVersion);
    }
   ...
   * @param tag
   */
  #getPatchedTagVersion(tag: string): string {
