...
The `<sbb-easter-egg>` component is a small hidden [Snake](<https://en.wikipedia.org/wiki/Snake_(video_game_genre)>) game rendered inside an [`<sbb-dialog>`](/angular/components/dialog/overview) so it can be shown as a modal on top of any page — a lightweight "easter egg" for playful moments.
...
```html
<sbb-button [sbbEasterEgg]="snake">Play Snake</sbb-button>
<sbb-easter-egg #snake="sbbEasterEgg"></sbb-easter-egg>
```
